...
{
    $user_id = $this->session->userdata('user_id');
    $this->db->select('DISTINCT a.timesheet_id, a.cheque_date, d.code, c.id, c.first_name, c.middle_name, c.last_name, d.tax_type, d.tax, SUM(d.amount) as total_amount', false);
    ...
    if ($emp_name !== 'All') {
        ...
        $this->db->where("(TRIM(CONCAT_WS(' ', c.first_name, c.middle_name, c.last_name)) LIKE '%$trimmed_emp_name%' OR TRIM(CONCAT_WS(' ', c.first_name, c.last_name)) LIKE '%$trimmed_emp_name%')");
    }
    if ($date) {
    ...
    $this->db->group_by('a.timesheet_id, d.code,a.cheque_date,  c.id, c.first_name, c.middle_name, c.last_name, d.tax_type, d.tax,d.amount');
    $query = $this->db->get();
   // echo $this->db->last_query(); die();
    return $query->result_array();
}
...
public function state_summary_employer($tax_choice = null, $selectState = null, $date = null, $emp_name = 'All', $taxType = null)
{
  //  echo $tax_choice;
    $user_id = $this->session->userdata('user_id');
    $this->db->select('DISTINCT a.timesheet_id, a.cheque_date, d.code, c.id, c.first_name, c.middle_name, c.last_name, d.tax_type, d.tax, SUM(d.weekly), SUM(d.biweekly), SUM(d.monthly), SUM(d.amount) as total_amount', false);
    $this->db->from('timesheet_info a');
    $this->db->join('info_payslip b', 'b.templ_name = a.templ_name');
    $this->db->join('employee_history c', 'c.id = b.templ_name');
    ...
    if ($date) {
        $dates = explode(' to ', $date);
        if (count($dates) == 2) {
            $start_date = $this->db->escape($dates[0]);
            $end_date = $this->db->escape($dates[1]);
